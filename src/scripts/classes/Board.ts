...
import { rawTasks, storageAvailable } from '@/scripts/global'

import type { BoardType } from '@/scripts/types'
...
export class Board implements BoardType {
  ...
  prependNumsToTasks(): void {
    this.tasks = rawTasks.map((task: string, i: number) => {
      let taskNum
      if (i + 1 > 9) {
        taskNum = `${i + 1}`
      } else {
  ...
  assignTasksToFlaps(): void {
    ...
    this.tasks.forEach((task: string, taskNumber: number) => {
      ...
      if (this.timer > this.interval) {
        ...
        this.flaps.forEach((flap, i) => {
          if (flap.targetChar) {
            ...
            const task = this.tasks[flap.task]
